...
public class ScrollContentView: UIView {
    ...
    lazy var collectionView: CustomGestureCollectionView = {
        ...
        collection.register(UICollectionViewCell.self, forCellWithReuseIdentifier: ScrollContentView.cellId)
        return collection
    }()
...
extension ScrollContentView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ScrollContentView.cellId, for: indexPath)
...
extension ScrollContentView: UIScrollViewDelegate {
    ...
    public func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        //HPrint(scrollView.contentOffset.x)
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //HPrint("progress:\(progress)------old:\(oldIndex)----currentIndex:\(currentIndex)")
        
        delegate?.contentViewMoveToIndex(fromIndex: oldIndex, toIndex: currentIndex, progress: progress)
        postCurrentIndexNotification(fromIndex: oldIndex, toIndex: currentIndex, progress: progress)
...
public protocol ContentViewDelegate: class {
    // MARK: -TODO: /** 此处默认实现后期有待优化，否则使用起来及其不方便 */
...
extension ScrollContentView {
    // 发布通知
    fileprivate func postCurrentIndexNotification(fromIndex: Int, toIndex: Int, progress: CGFloat) {
        
        if 1.0 == progress {// finish scrolling to next page
